...
if departments is not None:
    departments = departments.dropna().unique().tolist()
else:
    st.warning("Column 'department' not found in dataset.")
    departments = []
...
def map_group(qualification):
    ...
    elif q:  # Any other qualification
        return ["Group D"]
    else:
        return []
...
def generate_pdf_tabulated(jobs_df):
    ...
    for _, job in jobs_df.iterrows():
        ...
        group = str(job.get('group', '-')).capitalize() if 'group' in job else '-'
        department = str(job.get('department', '-')).capitalize() if 'department' in job else '-'
